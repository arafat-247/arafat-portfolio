...
class BuildTests(unittest.TestCase):
    ...
    def test_drafts_and_attribution(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            schema=json.loads(re.search(r'<script type="application/ld\+json">(.*?)</script>',story).group(1))
            self.assertEqual(schema['author'],[{'@type':'Person','name':'Staff Correspondent'}])
            self.assertEqual(schema['contributor']['name'],'Arafat Rahaman')
            ...
            self.assertIn('data-version="16.10.0"',reporting)
            ...
            self.assertIn('<title>Arafat Rahaman | Journalist at The Daily Star</title>',home)
            self.assertIn('"@type": "WebSite"',home)
            self.assertIn('assets/favicon.png',home)
            self.assertTrue((out/'about/index.html').is_file())
            self.assertIn('"@type": "ProfilePage"',(out/'about/index.html').read_text())
            ...
            story=(out/'stories/a-shared-report/index.html').read_text()
            self.assertIn('"@id": "https://arafatrahaman.com/#person"',story)
            self.assertIn('"url": "https://arafatrahaman.com/about/"',story)
