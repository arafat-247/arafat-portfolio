...
class BuildTests(unittest.TestCase):
    ...
    def test_drafts_and_attribution(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('class="portalpanel portal-reporting"',home)
            self.assertIn('class="portalpanel portal-opinion"',home)
            self.assertIn('class="portalpanel portal-thoughts"',home)
            self.assertIn('class="portalpanel portal-photography"',home)
            self.assertIn('class="portalart portalart-reporting"',home)
            self.assertLess(home.index('class="portalpanel portal-reporting"'),home.index('class="portalpanel portal-opinion"'))
            self.assertLess(home.index('class="portalpanel portal-opinion"'),home.index('class="portalpanel portal-thoughts"'))
            self.assertLess(home.index('class="portalpanel portal-thoughts"'),home.index('class="portalpanel portal-photography"'))
            self.assertNotIn('assets/home/reference-report.webp',home)
            self.assertNotIn('assets/home/reference-photo.webp',home)
            self.assertIn('Rajshahi University-nightscape-Journalist-Arafat-Rahaman (6).webp',home)
            self.assertNotIn('class="mobilefresh"',home)
            ...
            story_pages=list((out/'stories').glob('*/index.html'))
            self.assertTrue(story_pages)
            clean_story=next((p for p in story_pages if 'class="storyhead"' in p.read_text()),None)
            self.assertIsNotNone(clean_story)
            story_html=clean_story.read_text()
            self.assertIn('class="page reading"',story_html)
            self.assertIn('class="storyhead"',story_html)
            self.assertIn('class="storyfooter"',story_html)
            self.assertIn('class="storymain"',story_html)
            self.assertIn('assets/portraits/byline.webp',story_html)
            self.assertIn('Originally published by',story_html)
            self.assertIn('min read',story_html)
            self.assertIn('data-share',story_html)
            self.assertIn('data-print',story_html)
            ...
            self.assertIn('data-credit-view="byline" aria-pressed="true"',reporting)
            self.assertIn('data-credit-view="contribution" aria-pressed="false"',reporting)
            self.assertNotIn('name="credit"',reporting)
