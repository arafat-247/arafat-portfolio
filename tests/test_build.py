...
class BuildTests(unittest.TestCase):
    ...
    def test_drafts_and_attribution(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('<h1 id="recent-work-title">Latest journalism</h1>',home)
            self.assertIn('class="recentwork"',home)
            self.assertIn('class="recentgrid"',home)
            self.assertNotIn('Reporting public life with evidence and context.',home)
            self.assertNotIn('class="sectionnav"',home)
            self.assertNotIn('class="homestats"',home)
            self.assertNotIn('class="tile ',home)
