...
class BuildTests(unittest.TestCase):
    ...
    def test_drafts_and_attribution(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('class="editorialhero-photo-note"',home)
            self.assertIn('class="editorialhero-paper editorialhero-paper-note"',home)
            self.assertNotIn('Better journalism for a fairer, more equal Bangladesh.',home)
            self.assertNotIn('IDEAS<br>PEOPLE<br>POLICY<br>CHANGE',home)
            self.assertIn('>Email <b aria-hidden="true">→</b></a>',home)
            ...
            self.assertEqual(home.count('class="worktile-art"'),3)
            self.assertEqual(home.count('data-home-tile='),4)
            self.assertIn('class="worktile-number">01</em>',home)
            self.assertIn('class="worktile-keywords"',home)
            self.assertIn('View all work →',home)
            self.assertIn('People,<br>policy and<br>a more equal<br>Bangladesh.',home)
            self.assertIn('class="workgrid"',home)
            self.assertNotIn('class="homehero-copy"',home)
            self.assertNotIn('class="tile tile-reporting"',home)
            self.assertRegex(home,r'class="worktile worktile-photos"[^>]*>.*?<img ',msg='Photography tile should contain an image')
