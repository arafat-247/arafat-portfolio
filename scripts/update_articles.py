...
UA="Mozilla/5.0 (compatible; ArafatRahamanPortfolio/9.0; public-author-portfolio-indexer)"
...
def story_header():
    return '''<header class="masthead-v9"><div class="shell masthead-v9-inner"><a class="identity-v9" href="../../" aria-label="Arafat Rahaman home"><strong>Arafat Rahaman</strong><span>Staff Reporter</span></a><nav class="nav-v9 desktop-only" aria-label="Primary"><a href="../../">Home</a><a href="../../archive.html">Archive</a><a href="../../photography.html">Photography</a><a href="../../about.html">About</a><a href="../../contact.html">Contact</a></nav><div class="masthead-actions-v9"><a class="tip-link-v9 desktop-only" href="../../contact.html#send-a-tip">Send a tip</a><button class="menu-button-v9 mobile-only" type="button" data-menu-toggle aria-controls="nav-overlay" aria-expanded="false">Menu</button></div></div></header><div id="nav-overlay" class="nav-overlay-v9" data-mobile-nav hidden><div class="nav-overlay-v9-top shell"><a class="identity-v9 inverse" href="../../"><strong>Arafat Rahaman</strong><span>Staff Reporter</span></a><button type="button" data-menu-close>Close</button></div><nav class="nav-overlay-links-v9 shell" aria-label="Mobile navigation"><a href="../../"><span>01</span>Home</a><a href="../../archive.html"><span>02</span>Archive</a><a href="../../photography.html"><span>03</span>Photography</a><a href="../../about.html"><span>04</span>About</a><a href="../../contact.html"><span>05</span>Contact</a></nav><div class="nav-overlay-v9-foot shell"><a class="overlay-tip-v9" href="../../contact.html#send-a-tip">Have information? <strong>Send a news tip →</strong></a></div></div>'''

def story_footer():
    return '''<footer class="footer-monograph-v9 site-footer"><div class="shell footer-monograph-v9-main"><div><p>ARAFAT RAHAMAN</p><h2>Journalism,<br>documented.</h2></div><div class="footer-links-v9"><a href="../../">Home</a><a href="../../archive.html">Archive</a><a href="../../photography.html">Photography</a><a href="../../about.html">About</a><a href="../../contact.html">Contact</a></div><div class="footer-contact-v9"><span>Staff Reporter · Dhaka, Bangladesh</span><a href="mailto:arafat.mcj@yahoo.com">arafat.mcj@yahoo.com</a><div><a href="../../feed.xml">RSS</a></div></div></div><div class="shell footer-monograph-v9-bottom"><span>© Arafat Rahaman</span><a href="../../contact.html#send-a-tip">Send a news tip →</a></div></footer>'''
...
def generate_story_page(story,config,enhancements):
    ...
    if isinstance(hero,dict) and hero.get("base"):
        hero_html=f'<figure class="story-hero-v9 story-hero-media">{picture_markup(hero["base"],hero.get("alt") or f"Editorial illustration for {story["title"]}")}<figcaption><span>{html_lib.escape(hero.get("caption","") or "")}</span><span>{html_lib.escape(hero.get("credit","") or "")}</span></figcaption></figure>'
        og_image=abs_site(config,hero["base"]+".webp")
    else:
        art=story.get("art_url","")
        hero_html=f'<figure class="story-hero-v9 auto-story-art"><img src="../../{html_lib.escape(art,quote=True)}" alt="Original editorial illustration for {title}" loading="eager" decoding="async"><figcaption><span>Original editorial illustration created for this portfolio.</span></figcaption></figure>' if art else ""
        og_image=abs_site(config,art) if art else ""
    ...
    topic_explore=(f'<a href="../../archive.html?topic={topic_q}"><span>More on {topic}</span><b>→</b></a>' if topic and topic!="Public Affairs" else "")
    readmins=max(1,round(story.get('word_count',0)/220))
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#f1efe9"><meta name="robots" content="noindex,follow"><title>{title} — Arafat Rahaman</title><meta name="description" content="{excerpt}"><link rel="canonical" href="{html_lib.escape(source,quote=True)}"><link rel="icon" href="../../assets/favicon.svg" type="image/svg+xml"><link rel="alternate" type="application/rss+xml" href="../../feed.xml"><meta property="og:type" content="article"><meta property="og:title" content="{title}"><meta property="og:description" content="{excerpt}"><meta property="og:url" content="{html_lib.escape(local_abs,quote=True)}">{f'<meta property="og:image" content="{html_lib.escape(og_image,quote=True)}">' if og_image else ''}<script type="application/ld+json">{jsonld_text}</script><link rel="stylesheet" href="../../styles.css"></head><body class="story-page" data-root="../../" data-source-url="{html_lib.escape(source,quote=True)}" data-word-count="{story.get('word_count',0)}"><div id="reading-progress" class="reading-progress" hidden><span></span></div><a class="skip-link" href="#article">Skip to article</a>{story_header()}<main class="story-main-v9 shell"><nav class="breadcrumbs-v9 breadcrumbs" aria-label="Breadcrumb"><a href="../../archive.html">Archive</a><span>/</span><a href="../../archive.html?type={category_q}">{typ}</a></nav><header class="story-opening-v9 story-header"><div class="story-meta-v9 story-header-meta"><a href="../../archive.html?type={category_q}">{typ}</a>{topic_meta}<span>Published {html_lib.escape(pub)}</span>{updated}</div><h1>{title}</h1>{stand}<div class="story-byline-v9 story-byline"><span>By <strong>{byline}</strong></span><span>{readmins} min read</span><div class="story-tools-inline"><button class="story-tool" data-share="copy">Copy link</button><button class="story-tool" data-print>Print / Save PDF</button></div></div></header>{hero_html}<div class="story-reading-v9 story-layout"><article id="article" class="story-prose">{body}</article><aside class="story-sidebar" aria-label="Story tools"><div class="sidebar-stack-v9 sidebar-stack"><section class="story-rail-block-v9 side-card source-card"><p>ORIGINAL PUBLICATION</p><strong>The Daily Star</strong><time>{html_lib.escape(pub)}</time><a href="{html_lib.escape(source,quote=True)}" target="_blank" rel="noopener">Read original ↗</a></section><section class="story-rail-block-v9 side-card share-card"><p>SHARE / SAVE</p><div class="share-buttons"><button data-share="copy">Copy link</button><button data-share="whatsapp">WhatsApp</button><button data-share="facebook">Facebook</button><button data-share="x">X</button><button data-share="linkedin">LinkedIn</button><button data-print>Print / PDF</button></div></section><section class="story-rail-block-v9 side-card"><p>EXPLORE</p><div class="explore-links">{topic_explore}<a href="#related-stories"><span>Related stories</span><b>↓</b></a><a href="../../archive.html"><span>Full archive</span><b>→</b></a></div></section><section id="evidence-card" class="story-rail-block-v9 side-card" hidden><p>EVIDENCE &amp; CONTEXT</p><div class="evidence-links"></div></section></div></aside></div><div class="mobile-share-bar" aria-label="Share this story" hidden><button data-share="copy">Copy</button><button data-share="whatsapp">WhatsApp</button><button data-share="facebook">Facebook</button><button data-print>Print</button></div><section class="source-note-v9 source-end"><div><span>Original publication</span><strong>The Daily Star · {html_lib.escape(pub)}</strong></div><a href="{html_lib.escape(source,quote=True)}" target="_blank" rel="noopener">View original article ↗</a></section><section id="related-stories" class="related-section"></section><nav id="story-sequence" class="story-sequence" aria-label="Previous and next stories"></nav></main>{story_footer()}<script src="../../common.js"></script><script src="../../story.js"></script></body></html>'''
