...
class Builder:
    ...
    def story(self,a):
        ...
        stream_cf=clean(stream_label).casefold()
        ...
        content=(f'<article class="page reading"><header class="storyhead">'
                 ...
                 f'<div class="storyfooter"><div class="byline"><div><strong>{byline}</strong><span class="meta">{esc(date_label(a.get("date_published","")))}{update}</span></div></div>'
...
def build():
    ...
    for i,(key,number,title,description,keywords) in enumerate(destinations):
        ...
        if key=='photos':
            ...
            visual=f'<img class="worktile-image" src="{esc(image)}" alt="" width="640" height="420" loading="lazy" decoding="async">' if image else '<b class="worktile-art" aria-hidden="true"></b>'
        else:
