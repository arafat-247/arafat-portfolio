...
def build():
    ...
    for i,(key,title,description) in enumerate(destinations):
        ...
        tiles.append(f'<a class="tile tile-{key}" href="{href}" data-home-tile="{key}">{visual}<b class="tileno" aria-hidden="true">{i+1:02d}</b><span><strong>{esc(title)}</strong><small>{esc(description)}</small><em class="tileexplore">Explore →</em></span><i aria-hidden="true">→</i></a>')
    home_profile='<section class="homeprofile" aria-labelledby="home-profile-title"><img src="assets/portraits/byline.webp" alt="Black-and-white portrait of Arafat Rahaman" width="900" height="892" loading="eager" fetchpriority="high" decoding="async"><div><span>Journalist · Dhaka</span><h2 id="home-profile-title"><span>Arafat</span><em>Rahaman</em></h2><p>Reporting on education, governance, public accountability and social issues for The Daily Star.</p><nav><a href="about/">About me →</a><a href="mailto:'+esc(c.get('email',''))+'">Email →</a></nav></div><blockquote class="heroquote">People,<br>policy and<br>a more equal<br>Bangladesh.</blockquote></section>'
    home_header=''
