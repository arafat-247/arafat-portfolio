...
class Builder:
    ...
    def page(self,path,title,body,home=False,desc='',article_data=None):
        ...
        menu+=f'<a href="{prefix}photography.html">Photography</a><a href="{prefix}about.html">About</a><a href="{prefix}contact.html">Contact</a>'
        ...
        document=f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{esc(title)} — {esc(name)}</title><meta name="description" content="{esc(desc or c.get('description',''))}"><meta name="theme-color" content="#111820"><link rel="canonical" href="{esc(self.base+'/'+path)}"><link rel="icon" href="{prefix}assets/favicon.svg" type="image/svg+xml"><link rel="alternate" type="application/rss+xml" href="{prefix}feed.xml" title="Arafat Rahaman"><link rel="stylesheet" href="{prefix}portfolio.css?v={ASSET_VERSION}"><meta property="og:title" content="{esc(title)}"><meta property="og:description" content="{esc(desc or c.get('description',''))}"><meta property="og:url" content="{esc(self.base+'/'+path)}"><script type="application/ld+json">{schema}</script></head><body class="{'home' if home else 'inner'}" data-root="{prefix}" data-page="{page_key}" id="top"><a class="skip" href="#main">Skip to content</a><aside class="identity" aria-label="Profile and navigation"><a href="{prefix}index.html" aria-label="Arafat Rahaman homepage"><img class="portrait" src="{prefix}{esc(portrait)}" width="132" height="132" alt="Arafat Rahaman" decoding="async"></a><div class="social">{socials}</div><a class="name" href="{prefix}index.html">ARAFAT<br>RAHAMAN</a><p>Journalist at The Daily Star<br>Dhaka, Bangladesh</p><nav aria-label="Main navigation"><a href="{prefix}index.html">Portfolio</a><a href="{prefix}about.html">About me</a><a href="{prefix}contact.html">Contact</a></nav></aside><div class="right"><header class="mobilehead"><a class="mobilebrand" href="{prefix}index.html"><span class="mark" aria-hidden="true">A</span>Arafat Rahaman</a><button class="menutoggle" aria-expanded="false" aria-controls="mobile-menu">Menu</button></header><button class="menubackdrop" hidden aria-label="Close menu"></button><nav class="mobilemenu" id="mobile-menu" hidden aria-label="Mobile navigation"><div class="drawerhead"><strong>Explore</strong><button class="drawerclose" type="button" aria-label="Close menu">Close ×</button></div>{menu}<div class="menumeta">{socials}</div></nav><main id="main">{body}</main><footer><span>© {datetime.now().year} {esc(name)}</span><span>Dhaka, Bangladesh</span><a class="top" href="#top">Back to top ↑</a></footer></div><script src="{prefix}portfolio.js?v={ASSET_VERSION}" defer></script></body></html>'''
...
def build():
    ...
    b.page('index.html','Portfolio','<section class="homecontent"><header class="homeintro"><span>Selected paths through my work</span><h1>Portfolio</h1><p>Reporting, analysis, personal writing and photography.</p></header><div class="tiles">'+''.join(tiles)+'</div></section>',home=True)
    ...
    for p in photos:
        ...
        if not src:continue
        gallery+=f'<figure><button data-photo="{esc(src)}" aria-label="Open photograph: {esc(alt)}"><img src="{esc(src)}" alt="{esc(alt)}" loading="lazy" decoding="async"></button><figcaption>{esc(p.get("caption",""))}<small>{esc(p.get("location",""))}</small></figcaption></figure>'
    b.page('photography.html','Photography','<section class="page photopage"><header class="pageintro"><div><span class="eyebrow">Visual notes</span><h1>Photography</h1><p>People, places and everyday observations.</p></div></header><div class="photogrid">'+(gallery or '<p>No photographs published yet.</p>')+'</div><dialog id="photo-dialog"><button data-close-photo>Close photograph ×</button><img alt=""></dialog></section>')
    ...
    about=f'<section class="page aboutpage"><header class="pageintro"><div><span class="eyebrow">Profile</span><h1>About Arafat</h1><p>Journalism grounded in public interest, careful reporting and accountability.</p></div></header><div class="aboutlead"><figure><img src="{esc(safe_asset(c.get("portrait")))}" alt="Arafat Rahaman" decoding="async"></figure><div class="biocopy"><span class="eyebrow">Arafat Rahaman</span>{text_body(bio)}<a class="textlink" href="contact.html">Get in touch →</a></div></div><section class="coverage"><div><span class="eyebrow">Reporting areas</span><h2>What I cover</h2></div><ul>{areas}</ul></section><section class="recognition" id="recognition"><header><span class="eyebrow">Recognition</span><h2>Awards & fellowships</h2></header><ol>{awards}</ol></section></section>'
    b.page('about.html','About',about)
