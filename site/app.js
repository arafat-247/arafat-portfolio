(() => {
  ...
  function matches(article) {
    ...
    if (!q) return true;
    return [article.title, article.excerpt, article.section]
  ...
  function renderFeatured(article) {
    ...
    featuredSlot.innerHTML = `
      <a class="featured-card" href="${esc(article.url)}" target="_blank" rel="noopener">
        <div class="featured-number">01</div>
        <div>
          <div class="story-meta"><span>${esc(article.section || "Reporting")}</span><span>•</span><time>${esc(prettyDate(article.date))}</time></div>
  ...
  function renderStories() {
    ...
    grid.innerHTML = rest.map((article, idx) => `
      <a class="story-card" href="${esc(article.url)}" target="_blank" rel="noopener">
        <div class="story-meta"><span>${esc(article.section || "Reporting")}</span><span>•</span><time>${esc(prettyDate(article.date))}</time></div>
        <h3>${esc(article.title)}</h3>
        <p>${esc(article.excerpt || "")}</p>
        <div class="card-footer"><span>Original story</span><span class="arrow" aria-hidden="true">↗</span></div>
      </a>
    `).join("");
  ...
  async function init() {
    try {
      ...
      document.querySelector("#stat-latest").textContent = articles.length ? monthYear(articles[0].date) : "—";
