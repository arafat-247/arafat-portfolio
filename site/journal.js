(()=>{
 ...
 AR.optionalJson('data/journal-index.json',{articles:[]}).then(data=>{
   ...
   host.innerHTML=items.map(a=>{const u=a.local_url||`journal-story.html?slug=${encodeURIComponent(a.slug||'')}`;return `<article class="journal-item"><time>${AR.esc(AR.fmtDate(a.date_published||a.date))}</time><h3><a href="${AR.esc(u)}">${AR.esc(a.title)}</a></h3><a href="${AR.esc(u)}">↗</a></article>`}).join('');
 }).catch(()=>{});
})();
