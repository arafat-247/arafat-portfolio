window.AR=(()=>{
  ...
  const storyDate=s=>s.date_published||s.date||"";
  const categoryURL=s=>`${root()}archive.html?type=${encodeURIComponent(category(s))}`;
  const topicURL=s=>`${root()}archive.html?topic=${encodeURIComponent(topic(s))}`;
